...
export default function Coursedescription(pros) {
    ...
    const unregister = async () => {
        const raw_data_unregister = await fetch(`http://10.201.30.27/pms/backend/api/plugin/coursemember/${courses_id}`,
    ...
    useEffect(() => {
        async function get_description() {
            const raw_data_description = await fetch(`http://10.201.30.27/pms/backend/api/plugin/registercourse/${courses_id}`,{'credentials': "include"})
