...
export default function JoinCourses() {
    ...
    const register_course = (password) => fetch(`http://localhost:8080/api/plugin/registercourse/${courseid}/${password}`,{'method' : 'POST', 'credentials': "include"})
        .then(async (res) => {
            res = await res.json()
    ...
    useEffect(async ()=>{
        ...
        fetch(`http://localhost:8080/api/plugin/registercourse/${courseid}`,{'credentials': "include"})
        .then(async (res) => {
            res = await res.json()
