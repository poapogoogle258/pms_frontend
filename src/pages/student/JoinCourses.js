...
export default function JoinCourses() {
    ...
    const register_course = (password) => fetch(`http://10.201.30.27/pms/api/api/plugin/registercourse/${courseid}/${password}`,{'method' : 'POST', 'credentials': "include"})
        .then(async (res) => {
            res = await res.json()
    ...
    useEffect(async ()=>{
        ...
        fetch(`http://10.201.30.27/pms/api/api/plugin/registercourse/${courseid}`,{'credentials': "include"})
        .then(async (res) => {
            res = await res.json()
